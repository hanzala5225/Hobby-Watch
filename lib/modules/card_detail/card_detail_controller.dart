...
class CardDetailController extends GetxController {
  ...
  Future<void> showMarkSoldDialog(BuildContext context) async {
    ...
    final taxChargedController = TextEditingController();
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => MarkSoldDialog(
        ...
        taxChargedController: taxChargedController,
  ...
  Future<void> _doMarkSold(double soldPrice, bool soldOutsideEbay, double shippingCharge, double taxCharged) async {
    isMarkingSold.value = true;
    try {
      final updated = await _api.markAsSold(card.value.id, soldPrice,
          soldOutsideEbay: soldOutsideEbay, shippingCharge: shippingCharge, taxCharged: taxCharged);
      card.value = updated;
...
class MarkSoldDialog extends StatelessWidget {
  ...
  final TextEditingController taxChargedController;
  ...
  const MarkSoldDialog({
    ...
    required this.taxChargedController,
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Tax section — same visibility rule as Shipping:
                          // irrelevant once "Sold outside of eBay" is checked.
                          Obx(() {
                            if (soldOutsideEbay.value) return const SizedBox();
                            return Column(
                              mainAxisSize: MainAxisSize.min,
                              ...
                              children: [
                                SizedBox(height: 16.h),
                                Text('Tax charged to buyer',
                                    style: GoogleFonts.inter(fontSize: 13.sp, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                                SizedBox(height: 4.h),
                                Text('eBay applies its fee to the sold price + shipping + tax combined.',
                                    style: GoogleFonts.inter(fontSize: 10.sp, color: AppColors.textMuted)),
                                SizedBox(height: 8.h),
                                TextFormField(
                                  controller: taxChargedController,
                                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                                  style: GoogleFonts.inter(fontSize: 16.sp, fontWeight: FontWeight.w700, color: AppColors.textPrimary),
                                  decoration: InputDecoration(
                                    prefixText: '\$  ',
                                    prefixStyle: GoogleFonts.inter(fontSize: 14.sp, fontWeight: FontWeight.w600, color: AppColors.textMuted),
                                    hintText: '0.00',
                          ...
                          // Live fee preview
                          AnimatedBuilder(
                            animation: Listenable.merge([soldPriceController, shippingChargeController, taxChargedController]),
                            builder: (_, __) => Obx(() {
                              ...
                              final tax = soldOutsideEbay.value
                                  ? 0.0
                                  : (double.tryParse(taxChargedController.text.replaceAll(',', '.')) ?? 0.0);
                              final combined = soldPrice + shipping + tax;
                              // eBay's percentage fee applies to price + shipping + tax combined.
                              final percentFee = soldOutsideEbay.value ? 0.0 : combined * (card.ebayFeePercent / 100);
                              // Plus a flat per-order fee: $0.30 under $10, $0.40 at/above $10 —
                              // based on the COMBINED order total (price + shipping + tax), not
                              // sold price alone. Confirmed against eBay's actual fee breakdown:
                              // a $10.50 item with $2.34 shipping+tax ($12.84 combined) was charged
                              // the $0.40 tier, and the threshold check is against that $12.84, not
                              // the $10.50 item price. This is a client-side preview only — the
                              // backend resolves the authoritative amount from app_settings
                              ...
                              return Container(
                                ...
                                child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                    Text(soldOutsideEbay.value ? 'eBay fee' : 'eBay fee (${card.ebayFeePercent.toStringAsFixed(2)}% + \$${flatFee.toStringAsFixed(2)} flat)',
